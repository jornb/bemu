...
struct Lcd : MemoryRegion<0xFF40, LcdRegisters> {
    ...
    [[nodiscard]] bool get_background_and_window_enable() const { return get_bit(m_data.m_control, 0); }
    ...
    [[nodiscard]] bool get_object_enable() const { return get_bit(m_data.m_control, 1); }
    ...
    ///
    /// \return 16 or 8
    [[nodiscard]] u8 get_object_height() const { return get_bit(m_data.m_control, 2) ? 16 : 8; }
    ...
    [[nodiscard]] u16 get_background_tile_map_start_address() const {
        return get_bit(m_data.m_control, 3) ? 0x9C00 : 0x9800;
    }
    ...
    [[nodiscard]] u16 get_background_and_window_tile_data_start_address() const {
        return get_bit(m_data.m_control, 4) ? 0x8000 : 0x8800;
    }
    ...
    ///           be ignored.
    [[nodiscard]] bool get_window_enable() const { return get_bit(m_data.m_control, 5); }
    ...
    ///
    /// The map is a row-major 32x32 list of u8 tile IDs, creating a 256x256 image.
    [[nodiscard]] u16 get_window_tile_map_start_address() const {
        return get_bit(m_data.m_control, 6) ? 0x9C00 : 0x9800;
    }
    ...
    /// the first frame.
    [[nodiscard]] bool get_enable_lcd_and_ppu() const { return get_bit(m_data.m_control, 7); }

    PpuMode get_ppu_mode() const { return static_cast<PpuMode>(m_data.m_status & 0b11); }
    void set_ppu_mode(PpuMode mode) { m_data.m_status = (m_data.m_status & ~0b11) | static_cast<u8>(mode); }

    [[nodiscard]] bool is_horizontal_blank_interrupt_enabled() const { return get_bit(m_data.m_status, 3); }
    [[nodiscard]] bool is_vertical_blank_interrupt_enabled() const { return get_bit(m_data.m_status, 4); }
    [[nodiscard]] bool is_oam_interrupt_enabled() const { return get_bit(m_data.m_status, 5); }
    [[nodiscard]] bool is_ly_compare_interrupt_enabled() const { return get_bit(m_data.m_status, 6); }
};
