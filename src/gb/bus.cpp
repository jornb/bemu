...
u8 Bus::read_u8(const u16 address) const {
    ...
    if (0xFEA0 <= address && address <= 0xFEFF) {
        // Nintendo indicates use of this area is prohibited. This area returns $FF when OAM is blocked, and
        // otherwise the behavior depends on the hardware revision.
        //
...
u16 Bus::read_u16(const u16 address) const {
    const auto lo = read_u8(address);
    const auto hi = read_u8(address + 1);
    return combine_bytes(hi, lo);
}

void Bus::write_u16(const u16 address, const u16 value) {
